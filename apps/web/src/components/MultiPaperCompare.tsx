...
export default function MultiPaperCompare() {
  ...
  async function handleCompare() {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : t('toolPage.multiPaperCompare.couldNotCompare');
      setError(message);
      showError(message);
    } finally {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-16">
      <h1 className="font-serif text-2xl font-semibold text-navy">{t('tool.multiPaperCompare.name')}</h1>
      <p className="mt-2 text-ink-soft">
        {t('toolPage.multiPaperCompare.description')}
      </p>

      {!flags ? (
        <>
          <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <PickSlot label={t('toolPage.multiPaperCompare.paperA')} slot={slotA} onPick={() => setPickingSlot('a')} />
            <PickSlot label={t('toolPage.multiPaperCompare.paperB')} slot={slotB} onPick={() => setPickingSlot('b')} />
          </div>
          ...
          >
            {isComparing ? t('toolPage.multiPaperCompare.comparing') : t('toolPage.multiPaperCompare.comparePapers')}
          </button>
        </>
      ) : (
        <div className="mt-6">
          <CompareView
            leftLabel={t('toolPage.multiPaperCompare.paperALabel').replace('{name}', slotA.file?.name ?? '')}
            rightLabel={t('toolPage.multiPaperCompare.paperBLabel').replace('{name}', slotB.file?.name ?? '')}
            leftText={joinedA}
            rightText={joinedB}
            flags={flags}
            reportTitle={t('toolPage.multiPaperCompare.reportTitle')}
          ...
          >
            {t('toolPage.multiPaperCompare.compareDifferentPapers')}
          </button>
        </div>
      )}
