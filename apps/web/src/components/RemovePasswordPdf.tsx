...
export default function RemovePasswordPdf() {
  ...
  async function handleUnlock() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('toolPage.removePassword.couldNotRemove'));
    } finally {
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-16">
      <h1 className="font-serif text-2xl font-semibold text-navy">{t('tool.removePassword.name')}</h1>
      <p className="mt-2 text-gray-600">
        {t('toolPage.removePassword.description')}
      </p>
      <p className="mt-1 text-xs text-gray-400">
        {t('toolPage.removePassword.serverNote')}
      </p>
      ...
      {file && (
        <div className="mt-6 rounded-lg border border-gray-200 bg-white p-4">
          <label className="block text-sm font-medium text-gray-700">{t('toolPage.removePassword.currentPassword')}</label>
          <input
            type="password"
            value={password}
      ...
      >
        {isProcessing ? t('toolPage.removePassword.removing') : t('toolPage.removePassword.removeAndDownload')}
      </button>
    </div>
  );
}
